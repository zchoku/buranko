...
private:
  ...
  double r(double t,double w, double del);
  double drdt(double t,double w, double del);
...
double RungeKutta::r(double t,double w, double del)
{
  return l - a*cos(w*t + del);
}
double RungeKutta::drdt(double t, double w,double del)
{
  return a*w*sin(w*t + del);
}
...
//初期値
double v_0 = 0.;
double x_0 = 0.9;//初期値は、0.3 rad * 1.8 m ~ 50 cm くらいのところ
